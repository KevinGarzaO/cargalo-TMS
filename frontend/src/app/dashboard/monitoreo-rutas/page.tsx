...
import { ArrowLeft, Search, Gauge, MapPin, Navigation } from 'lucide-react';
...
const DRIVERS_CONFIG = [
  { id: 'D3', nombre: 'Jorge Ibáñez', vehiculo: 'Camión - GGG-444', origen: 'Macroplaza, Monterrey', destino: 'Parque San Nicolás, NL', proximaParada: 'Tienda Soriana (Entrega)', img: 'https://images.unsplash.com/photo-1570295999919-56ceb5ecca61?w=200&auto=format&fit=crop', initLat: 25.6689, initLng: -100.3101 },
];
    ...
    <div className="h-[calc(100vh-140px)] flex flex-col gap-6">
      <div className="flex items-end justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-[28px] font-extrabold text-[#0E2A3A] tracking-tight leading-none mb-2">Monitoreo de Rutas Monterrey</h1>
        ...
        <div className="flex bg-white p-1 rounded-xl border border-ink-100 shadow-sm">
          <div className="px-4 py-1.5 flex items-center gap-2 border-r border-ink-100"><Gauge size={14} className="text-[#F97316]" /><span className="text-xs font-bold text-ink-900">30 km/h</span></div>
          <div className="px-4 py-1.5 flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-orange-500 animate-pulse" /><span className="text-xs font-bold text-ink-500">Ruta Maestra Activa</span></div>
        </div>
      </div>
        ...
        <div className="w-80 border-r border-ink-100 flex flex-col bg-white z-10">
          <div className="p-4 border-b border-ink-100 bg-ink-50/30">
            <div className="relative">
              ...
              <input placeholder="Buscar ruta..." className="w-full bg-white border border-ink-200 rounded-lg pl-9 pr-4 py-2 text-xs outline-none focus:border-primary transition-all" />
            </div>
          </div>
          <div className="flex-1 overflow-y-auto divide-y divide-ink-50 panel-scroll">
            {drivers.map(d => (
              <div key={d.id} className={`p-4 hover:bg-ink-50 cursor-pointer transition-all ${selectedId === d.id ? 'bg-orange-50/30 border-l-4 border-l-orange-500' : ''}`} onClick={() => setSelectedId(d.id)}>
                <div className="flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-full border-2 overflow-hidden shrink-0 ${selectedId === d.id ? 'border-[#F97316]' : 'border-orange-400'}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-1.5 mt-0.5"><Gauge size={11} className="text-[#F97316]" /><span className="text-[10px] font-black text-[#F97316]">30 km/h</span></div>
                  </div>
                  {selectedId === d.id && <div className="w-2 h-2 rounded-full bg-[#F97316] animate-pulse shrink-0" />}
                </div>
                <div className="mt-2 flex items-center gap-1.5 bg-orange-50 p-2 rounded-lg border border-orange-100/50">
                  <div className="w-1.5 h-1.5 rounded-full bg-orange-500 animate-pulse" />
                  <span className="text-[9px] font-black text-orange-600 uppercase">Próxima: {d.proximaParada}</span>
                </div>
              </div>
            ))}
          </div>
          ...
          {isLoaded ? (
            ...
            >
              {selectedId && clippedPath.length > 0 && (
                <Polyline path={clippedPath} options={{ strokeColor: '#F97316', strokeWeight: 8, strokeOpacity: 0.95, geodesic: true, zIndex: 1000 }} />
              )}

              {destPos && (
                <Marker position={destPos} icon={{ path: 'M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7z M12 11.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z', fillColor: '#F97316', fillOpacity: 1, strokeWeight: 2, strokeColor: '#fff', scale: 1.5, anchor: new google.maps.Point(12, 22) }} />
              )}
              ...
              {drivers.map(d => (
                <OverlayView key={d.id} position={{ lat: d.lat, lng: d.lng }} mapPaneName={OverlayView.OVERLAY_MOUSE_TARGET}>
                  <div className="flex flex-col items-center -translate-x-1/2 -translate-y-[100%] cursor-pointer" style={{ pointerEvents: 'auto' }} onClick={e => { e.stopPropagation(); setSelectedId(d.id); }}>
                    <div className={`text-white text-[10px] font-black px-4 py-1.5 rounded-full mb-2 shadow-2xl transition-all duration-300 whitespace-nowrap ${selectedId === d.id ? 'bg-[#F97316] scale-110 ring-4 ring-white' : 'bg-orange-600/80'}`}>
                      {d.nombre.split(' ')[0]}<span className="ml-2 opacity-70">30 km/h</span>
                    </div>
                    <div className="w-4 h-4 rounded-full border-[3.5px] border-white shadow-2xl animate-pulse bg-orange-500" />
                  </div>
                </OverlayView>
              ))}
              ...
              {selectedDriver && (
                <InfoWindow position={{ lat: selectedDriver.lat, lng: selectedDriver.lng }} onCloseClick={clearSelection}>
                  <div className="min-w-[270px] p-0">
                    <div className="flex items-center gap-3 p-3 pb-2.5 border-b border-orange-100">
                      <div className="w-12 h-12 rounded-full border-[3px] border-orange-500 overflow-hidden shadow-md shrink-0">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-1.5 mt-1"><div className="w-1.5 h-1.5 rounded-full bg-orange-500 animate-pulse" /><span className="text-[10px] font-black text-orange-600">En ruta activa</span></div>
                      </div>
                    </div>
                    ...
                    <div className="px-3 pb-2">
                      <div className="flex items-start gap-2 bg-orange-50 rounded-xl p-2.5 border border-orange-100">
                        ...
                        <div><div className="text-[8px] font-black text-orange-500 uppercase tracking-wider mb-0.5">Próxima parada</div><div className="text-[11px] font-bold text-ink-800">{selectedDriver.proximaParada}</div></div>
                      </div>
                    </div>
...
export default function MonitoreoRutas() {
  return (
